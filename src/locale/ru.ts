...
export const ru: UIStrings = {
  ...
  language: {
    ...
    variantNote: { romaji: "Написание по Хепбёрну с удвоением долгих гласных (Moomoomiruku).{br}Ромадзи в стиле вапуро НЕ поддерживается: оно противоречило бы официальному написанию (Butterfree, а не Batafurii),{br}а решение этой и вытекающих из неё проблем пока выходит за рамки проекта." },
    generatedRomaji:
  ...
  output: {
    ...
    empty: "Выберите темы, категории или собственные списки, чтобы получить результат.",
  ...
  footer: {
    ...
    helpOut: "Хотите помочь проекту? Смело загляните в",
  ...
  coveragePage: {
    ...
    noteLabelLister: "Чтобы добавить язык, которого нет в списке вовсе, откройте свои [настройки Wikidata](https://www.wikidata.org/wiki/Special:Preferences#mw-prefsection-gadgets) и включите гаджет {i}labelLister{/i} на вкладке «Гаджеты»; тогда у каждого элемента появится «Labels list» (вверху справа, в разделе «Инструменты»), принимающий любой языковой код.",
    ...
    noteStale: "Учтите, что эта таблица получена из ручного дампа, поэтому показанное здесь покрытие может отставать от текущего состояния Wikidata на несколько недель или даже месяцев.",

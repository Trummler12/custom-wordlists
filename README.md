...
Each topic is one JSON file under `data/topics/**`, described by [`schema/topic.schema.json`](schema/topic.schema.json). A word is a plain string when it reads the same in every language, a `{short, long}` name pair, or localizes at the leaf (`{"en": …, "de": …}`) — only where a language actually differs from English. See `data/topics/animation/south-park.json` for a full example. The manifest the app loads (`data/index.json`) is generated by `npm run build`.

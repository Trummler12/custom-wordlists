...
`zh-Hans` and `zh-Hant` are two entries rather than one — a reader who wants Traditional should be able to say so — and both are now official languages, each with an interface dictionary and a picker slot, arriving because the data has them (the Pokémon lists carry both scripts). They are not among the 28 languages this roadmap ranks, since that ranking is the set skribbl.io itself supports and Chinese is not one of them. Being off the ranked list is not the same as being unofficial, and Chinese is the case that shows the difference.

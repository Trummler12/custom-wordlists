  ...
  topics/
    gaming/                 # category folder (nestable, arbitrarily deep)
      ...
      pokemon/
        ...
        en.json
    sports/
      olympia/              # category that ALSO holds a loose topic file
        athletes.json       # flat topic: id = file stem (no id-named folder)
        sports/
          de.json           # foldered topic living beside the loose file
          en.json
...
- A **topic** is either a **folder** that directly holds its JSON file(s) and no subfolders (**id = folder name**), or — when a category folder also has subfolders — a **loose JSON file** in it (**id = file stem**, marked `flat` in the manifest; language-neutral only). Folders **above** a topic are an **arbitrarily deep category path** (`gaming/`, `gaming/pokemon/`, `sports/olympia/`), kebab-case like ids. **Filename convention:** a **language code** (`de.json`, `en.json`) marks a translated variant; any **other filename** (`champions.json`, `athletes.json`) marks a single **language-neutral** list. `build-index` derives a topic's languages from the filenames — no redundant `neutral.json` wrapper.
- `index.json` is **built by a script** that walks `data/topics/**` and emits a lightweight list `{ id, category, flat?, title, langs, files, groupCount, wordCount, icon }` so the frontend renders the tree *without* downloading every topic. `category` is the "/"-joined path from `topics/`; `files` lists the topic's JSON filenames so the frontend can fetch a neutral topic whose file is content-named (`champions.json`), not `<id>.json`. `flat` topics are fetched from `topics/<category>/<file>` (no id-named folder), foldered ones from `topics/<category>/<id>/<file>`. The chosen language file is fetched only when the user expands/selects that topic → fast first load, scales to hundreds of topics.
...
- **Word entries** are usually plain strings, but may also be a **`{ short, long }` name pair** (e.g. `{ "short": "Stan", "long": "Stan Marsh" }`). A group with any pair shows a per-group **Names** dropdown (short / long / both); counts and output de-duplicate identical rendered strings. See `schema/topic.schema.json` (`$defs.word`).
- A topic may carry optional **`lastUpdated`** / **`lastChecked`** ISO dates (`YYYY-MM-DD`) so we can spot lists due for a currency re-check.

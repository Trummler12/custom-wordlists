...
// Topic data lives at the repo root (PLANNING §4.1), not in public/, so Vite
// neither serves nor copies it by default. This plugin bridges that: it serves
// /data/* from the root data/ folder in dev and copies data/ into the build
// output on `build`. The manifest (data/index.json) is generated by the build
// script before `vite build`, so the copy picks it up.

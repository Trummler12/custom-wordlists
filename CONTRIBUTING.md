...
2. Word lists live in [`data/topics/**`](https://github.com/Trummler12/custom-wordlists/tree/main/data/topics) — **one JSON file per topic**, described by [`schema/topic.schema.json`](schema/topic.schema.json).
   ...
   - `"omitted"` lists families of entries the list hides from its source — junk nobody could draw, or numbered copies of one drawable thing. Each rule is a whole-name glob (`*`, `?`, `[0-9]`) plus a `"reason"`, and matches an entry when *any* of its language forms does, so one rule covers `"Data Card 01"` and `"Datenkarte01"` alike. Add `"as"` where the family should leave a name behind that the source never had: `{ "match": "Datenkarte[0-9]*", "as": { "en": "Data Card", "de": "Datenkarte" }, "reason": "…" }`. **The entries stay in the file** — they are filtered on the way into the list, so a rule is always reversible and the app can show what was left out.
